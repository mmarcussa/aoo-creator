...
function renderReadyState(){const el=$("#readyState");if(!el)return;
  ...
  el.title=errors?"Validation errors block the export. Click to see them.":"Validation passes. Click to build the mod."}
...
$("#readyState").addEventListener("click",startBuild);
...
// One action behind both ways in: the toolbar button and the ready chip.
// The chip used to jump to the validation report even when it read "Ready to
// build", which is a dead end and reads as a button that does nothing.
function startBuild(){renderValidation();
...
// Ctrl+Z inside a text field belongs to the browser. Hijacking it there meant a
// writer taking back their last sentence lost every structural change instead.
